...
class BoardWorkList extends Component {
  ...
  getTableColumns = () => {
    ...
    const elements = elementList.filter(item => item.visiabled === 1 && item.relationColumnName) || [];
    return elements.map((item, index) => {
      return {
        ...
        render: (text, record) => {
          const { relationTabId, elementId, relationColumnName, defaultValue, elementFormat, basetype } = item;
