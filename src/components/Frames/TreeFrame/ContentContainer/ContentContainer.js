 ...
 * @Last Modified time: 2021-01-14 17:04:33
 * @Desc: 框架主体内容（左侧树框架）
 */
...
import {
  ...
  HIDDEN_ELEMENT_TYPE,
  SEARCH_SELECT_TYPE,
  NUMBER_COLUMN_TYPE, HIDE_IN_LIST_ELEMENT_TYPE,
...
import {
  ...
  controlAuthority,
  jsonParse,
  returnJumpMenuData, getFunctionComDetail, reducerVariableToArray, multiDataComparison,
  unique, getUserInfo, getVariableValueByTree, jumpFileOfCommon, compoundArithmetic, validMatchMultiTrigger,
} from '@/utils/common';
import styles from './ContentContainer.less';
import ModalBox from '@/components/common/ReminderBox/ReminderBox';
import { JumpFileDate } from '@/components/Frames/LeftListFrame/method';
...
class ContentContainer extends Component {
  ...
  // 获取页面的业务数据
  doFetchPageData = () => {
    const { tabInfo, prevEleObj = {}, treeNodeData = {}, prevSelectedRows, actionType, prevTabInfo } = this.props;
    ...
    this.fetchTabDropDownData('normal', () => {
      ...
      if (
        ...
        (actionType === 'addGroup') ||
        (actionType === 'addChild') ||
        (prevEleObj.basetype === 24 && !prevSelectedRows.length && listShowType.includes(prevTabInfo.showType))
      ) {
  ...
  fetchFormData = () => {
    const { elementMap = {} } = this.state;
    const { tabInfo = {}, pageFrameData, treeNodeData, advancedQueryMap, prevTabInfo, prevTabData, fromType } = this.props;
  ...
  fetchTabDropDownData = (type = 'normal', callback) => {
    ...
    // 下拉框控件列表
    const dropDownList = elementList.filter(item => (SELECT_ELEMETN_TYPE.includes(item.basetype) && !!item.mainTableName && item.elementFormat === typeid) || item.basetype === 15)
  ...
  getGlobalVarValues = ({
    ...
    if (varName.indexOf('@@GetUserProperties') > -1) {
      return getUserInfo(varName, userInfo);
    }
    // 加减乘除四则混合运算
  ...
  singleEleCheckTrigger = ({
    ...
    const { id: eleId, triggerSign, basetype: originEleBaseType, mainTableName: originEleMainTableName } = eleObj;
    // 该元素有配置触发器
    ...
    triggers.forEach(trigger => { // 【加载】的前端触发器事件类型
      ...
      if (!relationElementValue) { // 如果relationElementValue为空，则值不变
        valueObj = newFormData[targetEleId];
      }
      // 保留旧的控件用旧的触发器逻辑
      if (!triggerSign) {
        const beforeData = newFormData[targetEleId] || {};
        if (beforeData.id) { // 若该控件本身有值，则取原值
          valueObj = beforeData;
        }
      }
      // // 有隐藏项的控件需要id和name不同
      ...
      const getInfoById = (infoEleObj, infoValueObj, paramName) => {
        console.log(infoEleObj, infoValueObj, paramName);
        ...
        getSelectSearchInfoById(infoEleObj, infoValueObj.id, function(res) {
          ...
          if (SEARCH_SELECT_TYPE.concat([17]).includes(targetEle.basetype) && !isNaN(Number(targetVlue))) {
            getSelectSearchInfoById(targetEle, targetVlue, function(res) {
              newFormData[targetEle.id] = res.valueObj;
              that.setState({ formData: newFormData }, () => {
                targetEle.basetype !== 17 && that.singleEleCheckTrigger({
                  eleObj: targetEle,
                  newElementList,
                  infosObj,
                  newFormData,
      ...
      } else if (SEARCH_SELECT_TYPE.includes(targetEleType) && newFormData[targetEleId]) { // 目标控件为选择框
  ...
  doHttpRequest = (btn, column, record) => {
    ...
    triggers.forEach(trigger => {
      ...
      httpRequestList.forEach(httpRequest => {
        ...
        if (column) { // 如果column有值，就走行编辑中的逻辑，没有的话走正常的
  ...
  handleNormalBtn = (btn) => {
    ...
    const { triggerMap, prevTabInfo, tabInfo, prevTabData, userInfo, treeNodeData, tabList } = this.props;
    const curEleTrigger = triggerMap[btnId] || [];
  ...
  jumpMenu = (btn, type) => {
    ...
    const elementList = elementMap[tabInfo.id] || [];
    const fileData = returnJumpMenuData(functionData.id, data, elementList, passParams, userInfo, getValueFromGlobalVariables, type);
  ...
  getTabContent = (isSingleTab = false) => {
    const { stepList, formData, listDataObj, listQueryParams, tabDropDownData, listSelectedRows, originFormData, fileRelationList, elementMap = {}, editListDataObj, externalParameters } = this.state;
    const { form, actionType, tabData = {}, advancedQueryMap, tabInfo = {}, triggerMap, originElementMap, treeNodeData, tabList, originTabData, prevTabInfo = {}, prevTabData = [], filterMap, functionData, userInfo, afterSave, setPrevContainerState, prevEleObj, handleSetPrevCustomSelectList, matchTabList, otherParamsInCustomList, filePassList, setFrameState } = this.props;
    ...
    const renderTabItem = (tabItem) => {
          ...
          <div className={styles.content}>
            <Form>
              <DetailForm
                {...publicProps}
                // form={form}
                formData={this.getDefaultValueFromCurrentTab({ currentTabData: formData, elementMap, tabInfo })}
      ...
      if (showType === 1) { // 列表类型
        ...
        return (
          <div className={contentCls}>
            <iframe id='tree_iframe' src={`${jumpUrl}?${qsStringify(externalParameters)}`}
                    frameBorder={0}
  ...
  saveInList = (btn, record) => {
    ...
    // 有配置后台触发器
    const curEleTrigger = triggerMap[btnId] || [];
    const static_triggerIds = curEleTrigger.filter(item => item.triggerType === 2).map(item => item.id).join(',');
    const static_procedure_triggerids = curEleTrigger.filter(item => item.triggerType === 3).map(item => item.id).join(',');
  ...
  render() {
    const { loadingStatus, visiblePopup, popupTabInfo = {}, listRowData = {}, formData = {}, triggerPopupEle, listSelectedRows, handleSetCustomSelectList, otherParamsInCustomList } = this.state;
    ...
    return (
      <div className={styles.wrap}>
        <CSpin spinning={loadingStatus.data}>
          {fromType === 'inPopup' ? this.getTabContent(true) : (
            <Tabs
              className={tabCls}
