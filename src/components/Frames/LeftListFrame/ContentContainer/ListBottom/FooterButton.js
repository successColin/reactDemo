...
import ContentContainer from '@/components/Frames/LeftListFrame/ContentContainer/ContentContainer';
    ...
    return btnArr.map((item) => {
      ...
      if (this.validPermission(item)) {
        switch (basetype) {
          ...
          case 24:
            return <div className={btnSty} key={item.id} onClick={() => normalBtnInList(item)}>
              <div className={btnStyOne}>
                {/*<Icon className={`${Styles.addnewBtnIco} custom-color`} type="plus-square"/>*/}
                <span>{item.displayName}</span>
              </div>
