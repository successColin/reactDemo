...
import { Col, Tooltip, Row, Modal, Select, Input, Form, InputNumber, DatePicker } from 'antd';
...
import {
  ...
  NUMBER_COLUMN_TYPE, TIME_FORMATE_TYPES, BTN_IN_LIST_COL, BTN_IN_EDIT_LIST, SELECT_IN_LIST, SEARCH_SELECT_TYPE,
...
import { CSearch, CSpin, CInput, NormalSearchSelect } from '@/components/common/BasicWidgets';
import {
  compoundArithmetic,
...
// 重置模糊查询的数据
  ...
  render() {
    ...
    // let formData = this.props.formData;
    const { filterMap, fetchListData, updataState, selectedListArr = [], triggerMap, activeTreeNode, userInfo, elementMap, tabList, tabInfo, btnGroups, isTrue = true, tabDropDownData, formData } = this.props;
    ...
    let btnInList = showType === 5 ? BTN_ELEMENT_TYP_COPY.filter(item => item !== 7) : BTN_ELEMENT_TYP_COPY;
    ...
    return <div style={{ 'height': 'calc(100%)' }} id="id1" ref="getContentWidth">
      ...
      {isTrue ? <CTable
        className={btnArr.length || searchArr.length ? (checkboxType === 'radio' || checkboxType === 'checkbox' ? Styles.tableHeight3 : Styles.tableHeight) : Styles.tableHeight2}
        columns={newColumns}
